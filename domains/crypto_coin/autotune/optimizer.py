...
# Parâmetros que devem ser inteiros (usados como window no pandas)
INT_PARAMS = {"ma_fast", "ma_slow", "rsi_period", "bb_period"}
...
def _rand_param(key: str):
    ...
    val = float(round(np.random.choice(values), 2))
    return int(val) if key in INT_PARAMS else val
...
def mutate(ind: Individual, mutation_rate: float = 0.2) -> Individual:
    ...
    for key in PARAM_SPACE:
        if random.random() < mutation_rate:
            ...
            setattr(mutated, key, int(new_val) if key in INT_PARAMS else new_val)
