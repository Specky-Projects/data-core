...
def calc_breakout_score(ind: "Indicators", df: pd.DataFrame) -> float:
    """
    ...
    rsi_oversold_level = 35.0  # fallback
    try:
        from domains.crypto_coin.legacy.config import load_config
        cfg_tmp = load_config()
        rsi_oversold_level = cfg_tmp.rsi_oversold
    except Exception:
        pass
    if ind.rsi is not None:
        depth = rsi_oversold_level - ind.rsi
        if depth > 0:
...
def compute_indicators(df: pd.DataFrame, cfg) -> Optional[Indicators]:
    ...
    if has_hlv:
        ind.breakout_score = calc_breakout_score(ind, df)
