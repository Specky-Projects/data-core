...
class ExchangeConnector:
    ...
    def restore_paper_state(self, quote_balance: float | None, base_amount: float | None,
                            buy_price: float | None = None):
        """Restore paper balances after a process restart."""
        if not self.cfg.paper_trading:
            return
        if quote_balance is not None:
            self._paper_balance = float(quote_balance)
        if base_amount is not None:
            self._paper_asset = float(base_amount)
        if buy_price is not None:
            self._paper_buy_price = float(buy_price)

    # ── Paper trading interno ─────────────────────────────────

    # Slippage simulado: 0.05% por side (ordem market não fecha no preço exato)
    _SLIPPAGE = 0.0005

    def _paper_buy(self, quote_amount: float, price: float) -> dict:
        quote_amount = min(quote_amount, self._paper_balance)
        effective_price = price * (1 + self._SLIPPAGE)
        ...
        self._paper_buy_price = effective_price
        return {"price": effective_price, "amount": qty, "cost": quote_amount, "fee": fee}

    def _paper_sell(self, asset_amount: float, price: float) -> dict:
        effective_price = price * (1 - self._SLIPPAGE)
        gross = asset_amount * effective_price
        fee = gross * 0.001
        net = gross - fee
