...
Coolify-managed `poupi-frontend-baby` should be reached through Traefik. Current validated route:

```text
http://wsp5l6d144vs27lz7p37b1hk.65.109.239.250.sslip.io/health -> 200 {"status":"ok","service":"poupi-frontend-baby",...}
```

The frontend currently uses a generated `sslip.io` URL. Promote it to stable DNS/TLS only after the DNS owner confirms the target record.
