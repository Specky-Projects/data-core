...
class SignalOutcomeTracker:
    ...
    def run(self, limit: int = 200) -> dict:
        ...
        for analytics in pending:
            try:
                outcome = self._evaluate(analytics, now)
                if outcome is None:
                    skipped += 1
                    continue
                self.db.add(outcome)
    ...
    def _evaluate(
    ...
    ) -> TradingSignalOutcome | None:
        """Compute outcome for one analytics row.

        Returns None if the horizon window has not yet closed (signal too recent).
        """
        timeframe = analytics.timeframe or "1h"
        delta = _TIMEFRAME_DELTA.get(timeframe, timedelta(hours=1))
        horizon_end = analytics.calculated_at + delta * EVALUATION_HORIZON
        ...
        if horizon_end > now:
            return None  # horizon not yet closed

        # Fetch candles within the horizon window
        candles = (
        ...
        if not candles:
            return None  # no subsequent data available
